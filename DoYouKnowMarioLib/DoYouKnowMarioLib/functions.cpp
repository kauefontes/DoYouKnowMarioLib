...
void writeInEndOfFile(string fl, string content)
{
	string line, line2;
	ifstream arq;
	arq.open(fl, fstream::app);
	arq<<content;
	arq.close();
}

void writeInFile(string fl, string content)
{
	string line, line2;
	ifstream arq;
	arq.open(fl);
...
int renameFile(string fl)
{

}

int deleteFile(string fl)
{
	if( remove( fl ) != 0 )
